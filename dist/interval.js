...
const appRoot = root;
const manualKey = (horizon, side) => `cia.manual.${horizon}.${side}`;
...
const cents = (value) => value == null || !Number.isFinite(value) ? "—" : `${(value * 100).toFixed(1)}¢`;
const signedCents = (value) => value == null || !Number.isFinite(value) ? "—" : `${value >= 0 ? "+" : ""}${(value * 100).toFixed(1)}¢`;
...
const dateTimeLabel = (timestamp) => new Date(timestamp * 1000).toLocaleString("en-US", {
...
function baseParameters(horizon) {
    return new URLSearchParams({ asset: state.asset, market_type: state.marketType, horizon });
}
function withManual(parameters, horizon) {
    const manual = state.manual[horizon];
    if (manual.up != null)
        parameters.set("manual_up", String(manual.up));
    if (manual.down != null)
        parameters.set("manual_down", String(manual.down));
    return parameters;
}
async function refreshHorizon(horizon, persist = true) {
    ...
    const [analysis, payload] = await Promise.all([
        api(`/api/interval/live?${liveParameters}`),
        api(`/api/interval/presets?${presetParameters}`),
    ]);
...
function noTradeExplanation(analysis) {
    if (!analysis.no_trade_reasons.length)
        return "";
...
function candleChart(horizon, compact = false) {
    const chart = state.chart;
    const analysis = state.analyses[horizon];
    if (!chart || !analysis || chart.candles.length < 3)
        return `<div class="empty">Candlestick data unavailable</div>`;
    ...
    if (analysis.expected_low != null)
        prices.push(analysis.expected_low);
    if (analysis.expected_high != null)
        prices.push(analysis.expected_high);
    prices.push(analysis.reference_price);
    const minPrice = Math.min(...prices);
    const maxPrice = Math.max(...prices);
    const xTime = (timestamp) => left + (timestamp - firstTimestamp) / Math.max(60, lastTimestamp - firstTimestamp) * (width - left - right);
    const y = (price) => top + (maxPrice - price) / Math.max(1e-9, maxPrice - minPrice) * (height - top - bottom);
...
function contractSideCard(side, comparison) {
...
function contractSection(horizon) {
    const comparison = state.contracts[horizon];
    if (!comparison)
        return `<div class="empty">Loading live contract prices…</div>`;
...
function intervalPage(horizon) {
    const analysis = state.analyses[horizon];
    if (!analysis)
        return state.loading ? `<div class="empty">Loading exact fixed interval…</div>` : `<div class="empty">Interval data unavailable</div>`;
...
function presetCard(preset, horizon) {
...
function casesTable(title, cases) {
    if (!cases.length)
        return `<div class="empty">No cases available</div>`;
...
function backtestPanel() {
    if (state.backtestLoading)
        return `<section class="panel"><div class="empty">Running same-minute fixed-interval backtest…</div></section>`;
    const result = state.backtest;
    if (!result)
        return `<section class="panel"><div class="empty">Click a preset to see its current gap and historical same-minute directional results.</div></section>`;
...
function presetsPage() {
    ...
    const payload = state.presetPayloads[horizon];
    if (!payload)
        return `<div class="empty">Preset data unavailable</div>`;
...
function render() {
    const tabs = [["15m", "15 MIN"], ["1h", "1 HOUR"], ["presets", "PRESETS"], ["research", "RESEARCH"], ["data", "DATA"], ["settings", "SETTINGS"]];
    let body = "";
    if (state.error)
        body = `<div class="notice bad"><strong>Unable to load live data</strong><p>${escapeHtml(state.error)}</p></div>`;
    else if (state.tab === "15m")
        body = intervalPage("15m");
    else if (state.tab === "1h")
        body = intervalPage("1h");
    else if (state.tab === "presets")
        body = presetsPage();
...
async function runBacktest(presetId, horizon) {
    ...
    if (payload)
        parameters.set("elapsed_seconds", String(Math.max(60, payload.elapsed_seconds)));
    try {
        state.backtest = await api(`/api/interval/presets/${encodeURIComponent(presetId)}/backtest?${parameters}`);
    }
    catch (error) {
        state.error = error instanceof Error ? error.message : String(error);
    }
...
function bind() {
    document.querySelectorAll("[data-tab]").forEach((button) => button.addEventListener("click", () => {
        ...
        if (state.tab === "15m" || state.tab === "1h")
            state.presetHorizon = state.tab;
        render();
    }));
    ...
    document.querySelectorAll(".open-presets").forEach((button) => button.addEventListener("click", () => {
        state.presetHorizon = button.dataset.horizon;
        state.tab = "presets";
        render();
    }));
    document.querySelectorAll(".preset-card, .preset-mini").forEach((button) => button.addEventListener("click", () => {
        const presetId = button.dataset.preset;
        const horizon = button.dataset.horizon;
        if (presetId && horizon)
            void runBacktest(presetId, horizon);
    }));
    document.querySelectorAll(".preset-horizon").forEach((button) => button.addEventListener("click", () => {
        state.presetHorizon = button.dataset.horizon;
        state.selectedPreset = null;
        state.backtest = null;
        render();
    }));
    document.querySelectorAll(".manual-form").forEach((form) => form.addEventListener("submit", async (event) => {
        event.preventDefault();
        const horizon = form.dataset.horizon;
        ...
        if (manual.up == null)
            ...
            localStorage.setItem(manualKey(horizon, "up"), String(manual.up));
        if (manual.down == null)
            localStorage.removeItem(manualKey(horizon, "down"));
        else
            localStorage.setItem(manualKey(horizon, "down"), String(manual.down));
        await refreshHorizon(horizon, false);
        render();
    }));
    document.querySelectorAll(".clear-manual").forEach((button) => button.addEventListener("click", async () => {
        const horizon = button.dataset.horizon;
...
setInterval(() => void refreshHorizon("15m").then(render).catch((error) => { state.error = String(error); render(); }), Math.max(3, state.refresh15) * 1000);
setInterval(() => void refreshHorizon("1h").then(render).catch((error) => { state.error = String(error); render(); }), Math.max(5, state.refresh60) * 1000);
