...
if (!root)
    throw new Error("Missing #research-app");
const appRoot = root;
const pages = [
    ["overview", "Overview"],
...
const fmtNumber = (value, digits = 2) => numberValue(value) === null ? "—" : Number(value).toLocaleString("en-US", { maximumFractionDigits: digits });
const fmtPct = (value, digits = 1) => numberValue(value) === null ? "—" : `${(Number(value) * 100).toFixed(digits)}%`;
const fmtMoney = (value) => numberValue(value) === null ? "—" : Number(value).toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 2 });
const fmtDate = (value) => {
...
async function refresh() {
    ...
    try {
        ...
        const [strategies, datasets, experiments, overview, funnel] = await Promise.all([
            api("/api/research/strategies"),
            api("/api/research/datasets"),
            api("/api/research/experiments"),
            api("/api/research/overview"),
            api("/api/research/funnel"),
        ]);
...
async function loadExperiment(id, rerender = true) {
    const detail = await api(`/api/research/experiments/${encodeURIComponent(id)}`);
    const index = state.experiments.findIndex((item) => item.experiment_id === id);
    if (index >= 0)
        state.experiments[index] = detail;
    else
        state.experiments.unshift(detail);
...
function statusClass(status) {
    ...
    if (lower.includes("eligible") || lower.includes("candidate") || lower === "completed")
        return "good";
    if (lower.includes("failed") || lower.includes("overfit") || lower.includes("suspended") || lower === "failed")
        return "bad";
    return "warn";
}
...
function renderStrategies() {
    if (!state.strategies.length)
        return empty("No registered strategies", "The backend has not loaded the strategy registry.");
...
function experimentTable(experiments) {
    if (!experiments.length)
        return empty("NO COMPLETED EXPERIMENTS", "No experiment records exist yet.");
...
function renderFunnel() {
    if (!state.funnel.length)
        return empty("NO EXPERIMENT COUNTS", "The funnel is populated only from stored experiment records.");
...
function renderWalkForward() {
    ...
    if (!folds.length)
        return empty("NO WALK-FORWARD RESULTS", "Select a completed experiment with chronological folds.");
...
function renderStability() {
    ...
    if (!experiment || Object.keys(stability).length === 0)
        return empty("NO PARAMETER STABILITY RESULT", "Run multiple tested parameter sets to classify a robust plateau, fragile optimum, or unstable region.");
    return `<div class="card"><div class="section-title"><h3>Neighborhood classification</h3><span class="pill ${statusClass(String(stability.classification ?? ""))}">${escapeHtml(stability.classification)}</span></div><p>Positive neighbor ratio: ${fmtPct(stability.positive_neighbor_ratio)}</p><div class="heat-grid">${neighbors.length ? neighbors.map((item) => {
        ...
        const positive = Number(metrics.expectancy ?? 0) > 0;
...
function renderMultiple() {
    ...
    if (!result)
        return empty("NO MULTIPLE-TESTING RESULT", "Run a completed experiment with one or more parameter configurations.");
...
function renderBootstrap() {
    ...
    if (!runs.length)
        return empty("NO BOOTSTRAP RESULTS", "Complete an experiment with out-of-sample trades.");
...
function renderBaselines() {
    ...
    if (!entries.length)
        return empty("NO BASELINE COMPARISONS", "Complete an experiment to compare its out-of-sample result with simple cost-adjusted alternatives.");
...
function renderIntegrity() {
    if (!state.datasets.length)
        return empty("NO DATASETS", "Import a real UTC candle dataset. Dataset integrity and strategy-specific feed requirements are checked before every experiment.");
...
function renderUnavailable(title, explanation) {
...
function bindEvents() {
    appRoot.querySelectorAll("[data-page]").forEach((button) => button.addEventListener("click", () => { state.page = button.dataset.page; render(); }));
    appRoot.querySelector("[data-refresh]")?.addEventListener("click", () => { void refresh(); });
    appRoot.querySelector("[data-import]")?.addEventListener("click", () => (document.querySelector("#import-dialog"))?.showModal());
    appRoot.querySelector("[data-new-experiment]")?.addEventListener("click", () => {
        const dialog = document.querySelector("#experiment-dialog");
        prefillExperimentDates();
        dialog?.showModal();
    });
    document.querySelectorAll("[data-close-dialog]").forEach((button) => button.addEventListener("click", () => button.closest("dialog")?.close()));
    appRoot.querySelectorAll("[data-open-experiment]").forEach((button) => button.addEventListener("click", () => { const id = button.dataset.openExperiment; if (id)
        void loadExperiment(id); }));
    appRoot.querySelectorAll("[data-rerun]").forEach((button) => button.addEventListener("click", async () => {
        const id = button.dataset.rerun;
        if (!id)
            return;
        try {
            const result = await api(`/api/research/experiments/${encodeURIComponent(id)}/rerun`, { method: "POST" });
            state.pollingJobId = String(result.job_id);
            await refresh();
        }
        catch (error) {
    ...
    document.querySelector("#import-form")?.addEventListener("submit", async (event) => {
        ...
        try {
            await api("/api/research/datasets", { method: "POST", body: JSON.stringify(JSON.parse(String(data.get("payload") ?? "{}"))) });
    ...
    document.querySelector("#experiment-form")?.addEventListener("submit", async (event) => {
        ...
        const form = event.currentTarget;
        const data = new FormData(form);
        const dataset = state.datasets.find((item) => item.dataset_id === String(data.get("dataset_id")));
        if (!dataset)
            return;
        ...
        try {
            ...
            const result = await api("/api/research/experiments", { method: "POST", body: JSON.stringify(payload) });
            state.pollingJobId = String(result.job_id);
            form.closest("dialog")?.close();
...
function prefillExperimentDates() {
    const form = document.querySelector("#experiment-form");
    if (!form || !state.datasets.length)
        return;
    ...
    const localInput = (timestamp) => new Date(timestamp * 1000).toISOString().slice(0, 16);
    if (start)
        start.value = localInput(dataset.start_timestamp);
    if (end)
        end.value = localInput(dataset.end_timestamp + dataset.source_timeframe_minutes * 60);
}
function pollJob() {
    if (!state.pollingJobId)
        return;
    const jobId = state.pollingJobId;
    const tick = async () => {
        try {
            const job = await api(`/api/research/jobs/${encodeURIComponent(jobId)}`);
