...
const DEFAULT_15_WEIGHTS: WeightSettings = {
  momentum: 18,
  ema: 16,
  rsi: 10,
  macd: 11,
...
const DEFAULT_60_WEIGHTS: WeightSettings = {
  momentum: 14,
  ema: 20,
  rsi: 8,
  macd: 13,
...
const category = (
  ...
  status: CategoryScore["status"] = "Derived",
): CategoryScore => {
...
const unavailableCategory = (key: string, label: string, feature: string): CategoryScore => ({
  key,
  label,
  score: null,
  direction: "Unavailable",
...
const calculateDataQuality = (candles: Candle[], generatedAt: number): number => {
  ...
  for (let index = 0; index < window.length; index += 1) {
    ...
    if (!candle || ![candle.open, candle.high, candle.low, candle.close, candle.volume].every(Number.isFinite)) invalid += 1;
    if (index > 0) {
      const previous = window[index - 1];
      if (candle && previous && candle.time - previous.time > 90_000) gaps += 1;
    }
  }
...
const regimeFrom = (
  ...
  if (currentPrice > priorHigh) return "Breakout attempt — upside";
  if (currentPrice < priorLow) return "Breakout attempt — downside";
  if (Math.abs(extensionAtr) > 2 && (rsi14 > 70 || rsi14 < 30)) return "Momentum exhaustion risk";
  if (trendStrength > 1.2 && bullish) return "Strong bullish trend";
  if (trendStrength > 1.2 && bearish) return "Strong bearish trend";
  if (trendStrength > 0.5 && bullish) return "Weak bullish trend";
  if (trendStrength > 0.5 && bearish) return "Weak bearish trend";
  if (realizedVol60 < 0.0025) return "Low-volatility range";
  if (realizedVol60 > 0.0075) return "High-volatility range";
  return "Uncertain or transitioning";
};

const correlationPenaltyFrom = (contributions: SignalContribution[]): number => {
  const byKey = new Map(contributions.map((item) => [item.key, item]));
  const groupPenalty = (keys: Array<keyof WeightSettings>, maximum: number): number => {
  ...
  return clamp(
    groupPenalty(["momentum", "ema", "breakout", "candle"], 0.08) +
      groupPenalty(["rsi", "macd", "bollinger", "meanReversion"], 0.06),
    0,
    0.14,
  );
};
...
export function analyzeCandles(
  ...
  horizonMinutes: HorizonMinutes = 15,
  livePrice?: number,
  ...
  const horizon = settings.horizons[String(horizonMinutes) as "15" | "60"];
  ...
  for (let index = Math.max(1, closes.length - 120); index < closes.length; index += 1) {
    ...
    if (current !== undefined && previous !== undefined) oneMinuteReturns.push(percentReturn(previous, current));
  }
  ...
  if (direction === "Neutral") noTradeReason = "insufficient directional edge";
  else if (confidence < horizon.minimumConfidence) noTradeReason = "confidence below minimum";
  else if (Math.abs(extensionAtr) > 2.6) noTradeReason = "price is highly extended";
  else if (dataQuality < 75) noTradeReason = "data quality below threshold";
  const tradeState = noTradeReason
    ? `No Trade — ${noTradeReason}`
    : `${confidence >= 72 ? "Strong " : confidence < 62 ? "Weak " : ""}${direction === "Bullish" ? "Long" : "Short"}`;

  const byKey = new Map(contributions.map((item) => [item.key, item]));
  const pick = (...keys: Array<keyof WeightSettings>): SignalContribution[] => keys.map((key) => byKey.get(key)).filter((item): item is SignalContribution => Boolean(item));
...
export const cloneSettings = (settings: AnalyzerSettings): AnalyzerSettings =>
  JSON.parse(JSON.stringify(settings)) as AnalyzerSettings;
