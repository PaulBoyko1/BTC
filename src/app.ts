...
import {
  createTrackedForecast,
  loadBinaryConfig,
  loadSettings,
  loadStrategyConfig,
  loadTrackedForecasts,
  resolveTrackedForecasts,
  saveBinaryConfig,
  saveSettings,
  saveStrategyConfig,
  saveTrackedForecasts,
} from "./storage.js";
import type {
  AnalyzerSettings,
  BacktestSummary,
  BinaryBacktestConfig,
  BinaryBacktestSummary,
  BinaryLadderRow,
  BinarySide,
  EntryCadence,
  Forecast,
...
type TabName = "dashboard" | "strategies" | "backtests" | "binary" | "history" | "methodology";

const state: {
  market: MarketState | null;
  forecasts: Record<"15" | "60", Forecast | null>;
  quickBacktests: Record<"15" | "60", BacktestSummary | null>;
  strategyResult: StrategyBacktestSummary | null;
  binaryResult: BinaryBacktestSummary | null;
  settings: AnalyzerSettings;
  strategyConfig: StrategyConfig;
  binaryConfig: BinaryBacktestConfig;
  tracked: TrackedForecast[];
  activeTab: TabName;
  chartHorizon: HorizonMinutes;
  loading: boolean;
  error: string | null;
  settingsOpen: boolean;
  nextCandleRefreshAt: number;
  nextTickerRefreshAt: number;
} = {
  market: null,
...
const percentNumber = (value: number | null, digits = 1): string => value === null ? "—" : `${value.toFixed(digits)}%`;
const ratioPct = (value: number | null, digits = 1): string => value === null ? "—" : `${(value * 100).toFixed(digits)}%`;
...
const directionClass = (direction: string): string => direction.toLowerCase().replaceAll(" ", "-");

const ptTime = (timestamp: number, includeDate = false): string => new Date(timestamp).toLocaleString("en-US", {
  timeZone: TIMEZONE,
...
const ptParts = (timestamp: number): { hour: number; minute: number; second: number; label: string } => {
...
const countdownText = (target: number): string => {
...
const hourlyCountdown = (): string => {
  const parts = ptParts(Date.now());
  const remaining = (59 - parts.minute) * 60 + (60 - parts.second);
  return `${String(Math.floor(remaining / 60)).padStart(2, "0")}:${String(remaining % 60).padStart(2, "0")}`;
};

const statusText = (): string => state.loading ? "Loading" : state.error ? "Delayed" : "Live";

const forecastFor = (horizon: HorizonMinutes): Forecast | null => state.forecasts[String(horizon) as "15" | "60"];
...
function renderShell(): void {
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-tab]").forEach((button) => {
    button.addEventListener("click", () => {
      state.activeTab = button.dataset.tab as TabName;
      render();
    });
  });
  document.querySelector<HTMLButtonElement>("#refreshButton")?.addEventListener("click", () => void refreshAll(true));
  ...
  if (state.error && !market) {
    ...
    document.querySelector<HTMLButtonElement>("#retryButton")?.addEventListener("click", () => void refreshAll(true));
    return;
  }
  ...
  if (state.activeTab === "strategies") renderStrategyLab(content);
  if (state.activeTab === "backtests") renderBacktests(content);
  if (state.activeTab === "binary") renderBinaryLab(content);
  if (state.activeTab === "history") renderHistory(content);
  if (state.activeTab === "methodology") renderMethodology(content);
}

const tabButton = (tab: TabName, label: string): string =>
  `<button class="tab-button ${state.activeTab === tab ? "active" : ""}" data-tab="${tab}">${label}</button>`;
...
const errorView = (message: string): string => `
...
function renderDashboard(content: HTMLElement): void {
  const market = state.market!;
  ...
  const chart = document.querySelector<HTMLElement>("#priceChart");
  if (chart) renderPriceChart(chart, market.candles, chartForecast);
  document.querySelectorAll<HTMLButtonElement>("[data-chart-horizon]").forEach((button) => {
    button.addEventListener("click", () => {
      state.chartHorizon = Number(button.dataset.chartHorizon) as HorizonMinutes;
      render();
    });
  });
  document.querySelectorAll<HTMLButtonElement>("[data-track-horizon]").forEach((button) => {
    button.addEventListener("click", () => trackForecast(Number(button.dataset.trackHorizon) as HorizonMinutes));
  });
  document.querySelectorAll<HTMLElement>("[data-go-binary]").forEach((element) => {
    element.addEventListener("click", () => { state.activeTab = "binary"; render(); });
  });
}

const forecastCard = (forecast: Forecast): string => {
...
const categoryCard = (item: Forecast["categories"][number]): string => `
...
function renderStrategyLab(content: HTMLElement): void {
...
const numberField = (label: string, key: string, value: number, min: number, max: number, step: number, suffix = ""): string => `
...
const checkField = (key: string, label: string, checked: boolean): string => `
  <label class="toggle-field"><input type="checkbox" data-strategy-field="${key}" ${checked ? "checked" : ""}><span>${label}</span></label>
`;

function bindStrategyForm(): void {
  document.querySelectorAll<HTMLInputElement | HTMLSelectElement>("[data-strategy-field]").forEach((input) => {
    input.addEventListener("change", () => {
      const key = input.dataset.strategyField as keyof StrategyConfig;
  ...
  document.querySelector<HTMLButtonElement>("#runStrategy")?.addEventListener("click", () => {
  ...
  document.querySelector<HTMLButtonElement>("#resetStrategy")?.addEventListener("click", () => {
...
function updateStrategyField(key: keyof StrategyConfig, input: HTMLInputElement | HTMLSelectElement): void {
  if (input instanceof HTMLInputElement && input.type === "checkbox") {
    (state.strategyConfig[key] as boolean) = input.checked;
    return;
  }
  const numericKeys: Array<keyof StrategyConfig> = ["horizonMinutes", "minConfidence", "minAbsScore", "minVolumeZ", "minMovePct", "maxExtensionAtr", "takeProfitPct", "stopLossPct", "maxHoldMinutes", "feeBps", "slippageBps", "rapidMoveMinutes", "consolidationMinutes", "consolidationMaxAtr"];
  if (numericKeys.includes(key)) {
    (state.strategyConfig[key] as number) = Number(input.value);
    return;
  }
  if (key === "bias") state.strategyConfig.bias = input.value as TradeBias;
  else if (key === "cadence") state.strategyConfig.cadence = input.value as EntryCadence;
  else if (key === "session") state.strategyConfig.session = input.value as SessionFilter;
  else if (key === "strategyId") state.strategyConfig.strategyId = input.value;
}

const strategyResultHtml = (result: StrategyBacktestSummary | null): string => {
  if (!result) return `<section class="card empty-inline"><p>No custom result yet.</p><span>Choose an idea, personalize the rules, then run the backtest.</span></section>`;
  return `
...
function renderBacktests(content: HTMLElement): void {
  ...
  `;
  document.querySelectorAll<HTMLButtonElement>("[data-test-idea]").forEach((button) => {
    button.addEventListener("click", () => applyTestIdea(button.dataset.testIdea ?? ""));
  });
}

const baselineCard = (summary: BacktestSummary | null, title: string): string => {
  if (!summary) return `<article class="card empty-inline"><p>${title}</p><span>Not ready.</span></article>`;
  return `
...
const calibrationCard = (summary: BacktestSummary | null): string => {
  if (!summary) return `<article class="card"></article>`;
  return `
...
const testIdea = (id: string, name: string, detail: string): string => `
  <article><h3>${name}</h3><p>${detail}</p><button data-test-idea="${id}">Load and test</button></article>
`;

function applyTestIdea(id: string): void {
  if (id === "binary-60") {
  ...
  const base: StrategyConfig = { ...DEFAULT_STRATEGY_CONFIG, feeBps: state.settings.feeBps, slippageBps: state.settings.slippageBps };
  if (id === "strict-15") state.strategyConfig = { ...base, strategyId: "composite", minConfidence: 65, minAbsScore: 0.18, cadence: "Every 5 minutes", maxExtensionAtr: 2, maxHoldMinutes: 15 };
  if (id === "hour-trend") state.strategyConfig = { ...base, strategyId: "trend-continuation", horizonMinutes: 60, cadence: "Top of hour", maxHoldMinutes: 60, takeProfitPct: 0.6, stopLossPct: 0.4 };
  if (id === "rapid-reversal") state.strategyConfig = { ...base, strategyId: "rapid-reversal", minMovePct: 0.18, rapidMoveMinutes: 4, consolidationMinutes: 4, consolidationMaxAtr: 1.1, maxHoldMinutes: 15 };
  if (id === "ema-pullback") state.strategyConfig = { ...base, strategyId: "ema-pullback", requireEmaAlignment: true, requireVolumeConfirmation: true, maxHoldMinutes: 15 };
  if (id === "range-breakout") state.strategyConfig = { ...base, strategyId: "range-breakout", minVolumeZ: 0.5, requireVolumeConfirmation: true, maxHoldMinutes: 15 };
  saveStrategyConfig(state.strategyConfig);
  if (state.market) state.strategyResult = runStrategyBacktest(state.market.candles, state.settings, state.strategyConfig);
  state.activeTab = "strategies";
  render();
}

function renderBinaryLab(content: HTMLElement): void {
  const market = state.market!;
...
const binaryNumberField = (label: string, key: string, value: number, min: number, max: number, step: number, suffix = ""): string => `
...
`;
const binarySelectField = (label: string, key: string, values: string[], selected: string): string => `
...
`;
const binaryLadderRow = (row: BinaryLadderRow): string => `
...
function bindBinaryControls(): void {
  document.querySelectorAll<HTMLInputElement | HTMLSelectElement>("[data-binary-field]").forEach((input) => {
    input.addEventListener("change", () => {
      const key = input.dataset.binaryField;
      if (key === "strikeGap") state.settings.binaryStrikeGap = Number(input.value);
      else if (key === "levelCount") state.settings.binaryLevelCount = Number(input.value);
      else if (key === "side") state.binaryConfig.side = input.value as BinarySide;
      else if (key && key in state.binaryConfig) (state.binaryConfig as unknown as Record<string, number>)[key] = Number(input.value);
      saveSettings(state.settings);
      saveBinaryConfig(state.binaryConfig);
      render();
    });
  });
  document.querySelector<HTMLButtonElement>("#runBinaryBacktest")?.addEventListener("click", () => {
    if (!state.market) return;
  ...
  document.querySelector<HTMLButtonElement>("#resetBinary")?.addEventListener("click", () => {
...
const binaryResultHtml = (result: BinaryBacktestSummary | null): string => {
  if (!result) return `<section class="card empty-inline"><p>No hourly contract test yet.</p><span>Set the entry minute, strike offset, side, hypothetical price, and minimum edge.</span></section>`;
  return `
...
function renderHistory(content: HTMLElement): void {
  ...
  `;
  document.querySelector<HTMLButtonElement>("#clearHistory")?.addEventListener("click", () => {
    state.tracked = [];
...
function renderMethodology(content: HTMLElement): void {
  content.innerHTML = `
...
const statCard = (label: string, value: string, detail: string): string => `
  <article class="card stat-card"><span>${label}</span><strong>${value}</strong><small>${detail}</small></article>
`;

function currentPacificHourOpen(candles: MarketState["candles"]): number | null {
  const now = ptParts(Date.now());
  for (let index = candles.length - 1; index >= 0; index -= 1) {
    const candle = candles[index];
    if (!candle) continue;
    const parts = ptParts(candle.time);
    if (parts.hour === now.hour && parts.minute === 0) return candle.open;
    if (parts.hour !== now.hour && index < candles.length - 70) break;
  }
...
function settingsDrawer(): string {
  const labels: Record<keyof WeightSettings, string> = {
    momentum: "Momentum",
    ema: "EMA structure",
    rsi: "RSI",
    macd: "MACD",
  ...
  const horizonSection = (horizon: "15" | "60"): string => {
    ...
    return `
      <details class="settings-group" ${horizon === "15" ? "open" : ""}><summary>${horizon === "15" ? "15-minute" : "1-hour"} model</summary>
        ${Object.entries(config.weights).map(([key, value]) => `<label class="setting-row"><span>${labels[key as keyof WeightSettings]}</span><input type="number" min="0" max="50" step="1" data-setting-weight="${horizon}:${key}" value="${value}"></label>`).join("")}
...
function bindSettingsEvents(): void {
  ...
  const close = (): void => { state.settingsOpen = false; render(); };
  document.querySelector<HTMLElement>("#drawerBackdrop")?.addEventListener("click", close);
  document.querySelector<HTMLButtonElement>("#closeSettings")?.addEventListener("click", close);
  document.querySelector<HTMLButtonElement>("#restoreDefaults")?.addEventListener("click", () => {
  ...
  document.querySelector<HTMLButtonElement>("#saveSettings")?.addEventListener("click", () => {
    ...
    document.querySelectorAll<HTMLInputElement>("[data-setting-weight]").forEach((input) => {
      const [horizon, key] = (input.dataset.settingWeight ?? "").split(":") as ["15" | "60", keyof WeightSettings];
      next.horizons[horizon].weights[key] = Math.max(0, Number(input.value) || 0);
    });
    document.querySelectorAll<HTMLInputElement>("[data-horizon-setting]").forEach((input) => {
      const [horizon, key] = (input.dataset.horizonSetting ?? "").split(":") as ["15" | "60", keyof AnalyzerSettings["horizons"]["15"]];
      if (key !== "weights") (next.horizons[horizon][key] as number) = Number(input.value);
    });
    next.historyMinutes = Number(document.querySelector<HTMLSelectElement>("#historyMinutes")?.value ?? next.historyMinutes);
    next.refreshSeconds = Math.max(10, Number(document.querySelector<HTMLInputElement>("#refreshSeconds")?.value ?? next.refreshSeconds));
    next.feeBps = Math.max(0, Number(document.querySelector<HTMLInputElement>("#feeBps")?.value ?? next.feeBps));
    next.slippageBps = Math.max(0, Number(document.querySelector<HTMLInputElement>("#slippageBps")?.value ?? next.slippageBps));
    next.binaryStrikeGap = Math.max(25, Number(document.querySelector<HTMLInputElement>("#binaryStrikeGap")?.value ?? next.binaryStrikeGap));
    next.binaryLevelCount = Math.max(1, Number(document.querySelector<HTMLInputElement>("#binaryLevelCount")?.value ?? next.binaryLevelCount));
    ...
    saveSettings(state.settings);
    state.settingsOpen = false;
    if (historyChanged) void refreshAll(true);
    else { recalculateAll(); render(); showToast("Settings saved and models recalculated."); }
  });
}
...
function recalculateAll(): void {
  ...
  if (state.strategyResult) state.strategyResult = runStrategyBacktest(state.market.candles, state.settings, state.strategyConfig);
  if (state.binaryResult) state.binaryResult = runBinaryBacktest(state.market.candles, state.settings, state.binaryConfig);
}

function trackForecast(horizon: HorizonMinutes): void {
  ...
  if (duplicate) { showToast("This forecast is already being tracked.", true); return; }
...
async function refreshAll(manual = false): Promise<void> {
  ...
  try {
    ...
    saveTrackedForecasts(state.tracked);
    if (!state.strategyResult) state.strategyResult = runStrategyBacktest(market.candles, state.settings, state.strategyConfig);
...
async function refreshTickerOnly(): Promise<void> {
  if (!state.market || state.loading || state.settingsOpen || userIsEditing() || Date.now() < state.nextTickerRefreshAt) return;
  try {
...
function render(): void { renderShell(); }

window.setInterval(() => {
  const clock = document.querySelector<HTMLElement>("#pacificClock");
  if (clock) clock.textContent = ptTime(Date.now());
  const hour = document.querySelector<HTMLElement>("#hourlyCountdown");
  if (hour) hour.textContent = hourlyCountdown();
  document.querySelectorAll<HTMLElement>("[data-countdown]").forEach((element) => {
    const horizon = Number(element.dataset.countdown) as HorizonMinutes;
    const forecast = forecastFor(horizon);
    if (forecast) element.textContent = countdownText(forecast.targetTime);
  });
}, 1_000);
