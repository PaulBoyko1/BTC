type Tab = "15m" | "1h" | "chart" | "setups" | "research" | "data" | "settings";
...
const state: {
  ...
  loading: boolean;
  error: string | null;
  researchResult: JsonObject | null;
  refresh15: number;
  refresh60: number;
} = {
  tab: "15m",
...
function query(horizon: Horizon, persist = true): string {
...
async function refreshAnalysis(horizon: Horizon): Promise<void> {
  const analysis = await api<Analysis>(query(horizon));
...
function probabilityValue(analysis: Analysis, side: "up" | "down"): string {
  const value = side === "up" ? analysis.up_probability : analysis.down_probability;
  return value == null ? "—" : pct(value, 0);
}
function probabilityLabel(analysis: Analysis): string {
  if (analysis.probability_state === "calibrated") return "CALIBRATED OUT-OF-SAMPLE ODDS";
  if (analysis.probability_state === "stale") return "DATA STALE — ODDS SUPPRESSED";
  return "MODEL SCORE — INSUFFICIENT DATA FOR CALIBRATED ODDS";
}
function statusBadge(analysis: Analysis): string {
...
function factorSection(title: string, factors: Factor[]): string {
...
function intervalPage(horizon: Horizon): string {
  ...
  if (!analysis) return state.loading ? `<div class="empty">Loading fixed interval…</div>` : `<div class="empty">INSUFFICIENT DATA</div>`;
  const mode = horizon === "15m" ? "15-Minute" : "One-Hour";
  const feature = analysis.feature_snapshot;
  const flow = typeof feature.flow === "object" && feature.flow !== null ? feature.flow as JsonObject : {};
...
function chartPage(): string {
  ...
  if (!chart || chart.candles.length < 2) return `<div class="empty">Chart data unavailable</div>`;
  ...
  const x = (index: number): number => 40 + index / Math.max(1, candles.length - 1) * (width - 70);
  const y = (price: number): number => 25 + (maxPrice - price) / Math.max(1e-9, maxPrice - minPrice) * (height - 60);
...
async function loadSetups(): Promise<void> {
  ...
  for (const asset of assets) {
    for (const horizon of ["15m", "1h"] as Horizon[]) {
      const parameters = new URLSearchParams({ asset, market_type: state.marketType, horizon, persist: "false" });
      try { rows.push(await api<Analysis>(`/api/interval/live?${parameters}`)); } catch { /* keep failed assets absent */ }
    }
  }
...
function setupsPage(): string {
  const grouped = new Map<string, Partial<Record<Horizon, Analysis>>>();
  for (const row of state.setupRows) {
    const entry = grouped.get(row.asset) ?? {};
    entry[row.horizon] = row;
    grouped.set(row.asset, entry);
  }
  if (!grouped.size) return `<div class="empty"><strong>SETUP SCANNER NOT LOADED</strong><br><button id="load-setups">Load BTC and ETH</button></div>`;
...
function render(): void {
  const tabs: Array<[Tab, string]> = [["15m", "15 MIN"], ["1h", "1 HOUR"], ["chart", "CHART"], ["setups", "SETUPS"], ["research", "RESEARCH"], ["data", "DATA"], ["settings", "SETTINGS"]];
  let body = "";
  ...
  else if (state.tab === "chart") body = chartPage();
  else if (state.tab === "setups") body = setupsPage();
...
function bind(): void {
  document.querySelectorAll<HTMLButtonElement>("[data-tab]").forEach((button) => button.addEventListener("click", () => {
    ...
    if (state.tab === "setups" && !state.setupRows.length) void loadSetups();
    render();
  }));
  document.querySelector<HTMLSelectElement>("#asset-select")?.addEventListener("change", async (event) => {
    ...
    try { await Promise.all([refreshAnalysis("15m"), refreshAnalysis("1h"), refreshChart()]); } catch (error) { state.error = String(error); } finally { state.loading = false; render(); }
  });
  ...
  document.querySelector<HTMLButtonElement>("#load-setups")?.addEventListener("click", () => void loadSetups());
  document.querySelectorAll<HTMLButtonElement>(".asset-open").forEach((button) => button.addEventListener("click", async () => {
    state.asset = button.dataset.asset ?? state.asset; state.tab = "15m"; await initialLoad();
  }));
  document.querySelector<HTMLFormElement>("#settings-form")?.addEventListener("submit", (event) => {
    ...
    localStorage.setItem("cia.timezone", state.timezone); localStorage.setItem("cia.refresh15", String(state.refresh15)); localStorage.setItem("cia.refresh60", String(state.refresh60)); localStorage.setItem("cia.market", state.marketType);
    render();
  });
  document.querySelector<HTMLFormElement>("#ob-form")?.addEventListener("submit", async (event) => {
    event.preventDefault();
    const form = new FormData(event.currentTarget as HTMLFormElement);
    const simulations = Number(form.get("simulations") ?? 1000);
    const seed = Number(form.get("seed") ?? 42);
    state.researchResult = { status: "running" }; render();
    try {
      state.researchResult = await api<JsonObject>("/api/interval/order-blocks/research", {
        method: "POST",
        body: JSON.stringify({
    ...
    } catch (error) { state.researchResult = { error: error instanceof Error ? error.message : String(error) }; }
    render();
  });
}
...
setInterval(() => {
  ...
  const local = document.querySelector<HTMLElement>("#clock-local"); if (local) local.textContent = new Date().toLocaleTimeString("en-US", { timeZone: state.timezone });
  const utc = document.querySelector<HTMLElement>("#clock-utc"); if (utc) utc.textContent = utcLabel();
}, 1000);
