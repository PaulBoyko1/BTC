...
const pacificParts = (timestamp: number): PacificParts => {
...
export const DEFAULT_BINARY_CONFIG: BinaryBacktestConfig = {
  strikeGap: 100,
  strikeOffsetSteps: 0,
  entryMinute: 10,
  side: "Best edge",
...
const classifyActual = (entry: number, exit: number, neutralThresholdPct: number): Direction => {
  const movePct = entry === 0 ? 0 : ((exit - entry) / entry) * 100;
  if (movePct > neutralThresholdPct) return "Bullish";
  if (movePct < -neutralThresholdPct) return "Bearish";
  return "Neutral";
};

const accuracy = (rows: BacktestResultRow[]): number | null =>
  rows.length === 0 ? null : rows.filter((row) => row.correct).length / rows.length;

const makeBucket = (label: string, rows: BacktestResultRow[]): ConfidenceBucket => ({
  label,
  total: rows.length,
...
export function runBacktest(
  ...
  const warmup = 130;
  const horizonSettings = settings.horizons[String(horizonMinutes) as "15" | "60"];
...
const cadenceMatches = (timestamp: number, cadence: StrategyConfig["cadence"]): boolean => {
  const { minute } = pacificParts(timestamp);
  if (cadence === "Every minute") return true;
  if (cadence === "Every 5 minutes") return minute % 5 === 0;
  if (cadence === "New 15-minute block") return minute % 15 === 0;
  return minute === 0;
};

const sessionMatches = (timestamp: number, session: SessionFilter): boolean => {
  if (session === "All hours") return true;
  const { hour } = pacificParts(timestamp);
  if (session === "Pacific morning") return hour >= 5 && hour < 12;
  if (session === "Pacific afternoon") return hour >= 12 && hour < 17;
  if (session === "Pacific evening") return hour >= 17 && hour < 22;
  return hour >= 22 || hour < 5;
};

const biasMatches = (side: "Long" | "Short", bias: TradeBias): boolean => bias === "Both" || bias === side;

const latestHourCandles = (history: Candle[]): Candle[] => {
...
const rapidMoveSignal = (history: Candle[], forecast: Forecast, config: StrategyConfig, continuation: boolean): "Long" | "Short" | null => {
  ...
  if (history.length < rapid + consolidation + 2) return null;
  ...
  if (!start || !moveEnd || consolidationCandles.length < consolidation) return null;
  const movePct = start.close === 0 ? 0 : ((moveEnd.close - start.close) / start.close) * 100;
  if (Math.abs(movePct) < Math.max(0.01, config.minMovePct)) return null;
  const range = Math.max(...consolidationCandles.map((candle) => candle.high)) - Math.min(...consolidationCandles.map((candle) => candle.low));
  if (forecast.indicators.atr14 <= 0 || range / forecast.indicators.atr14 > config.consolidationMaxAtr) return null;
  ...
  if (consolidationVolume > initialVolume * 1.15) return null;
  const initialSide: "Long" | "Short" = movePct > 0 ? "Long" : "Short";
  if (continuation) return initialSide;
  return initialSide === "Long" ? "Short" : "Long";
};

const signalForStrategy = (
  history: Candle[],
  forecast: Forecast,
  ...
  const scoreSide: "Long" | "Short" | null = forecast.direction === "Bullish" ? "Long" : forecast.direction === "Bearish" ? "Short" : null;
  ...
  switch (config.strategyId) {
    ...
    case "trend-continuation":
      if (i.ema9 > i.ema21 && i.ema21 > i.ema50 && i.return15m > 0) side = "Long";
      if (i.ema9 < i.ema21 && i.ema21 < i.ema50 && i.return15m < 0) side = "Short";
      break;
    case "ema-pullback": {
      ...
      if ((nearFast || nearSlow) && i.ema21 > i.ema50 && current.close >= i.ema21 && i.rsi14 >= 42) side = "Long";
      if ((nearFast || nearSlow) && i.ema21 < i.ema50 && current.close <= i.ema21 && i.rsi14 <= 58) side = "Short";
      break;
    }
    case "range-breakout":
      if (current.close > i.priorRangeHigh) side = "Long";
      if (current.close < i.priorRangeLow) side = "Short";
      break;
    case "sweep-reclaim":
      if (current.low < i.priorRangeLow && current.close > i.priorRangeLow) side = "Long";
      if (current.high > i.priorRangeHigh && current.close < i.priorRangeHigh) side = "Short";
      break;
    case "mean-reversion":
      if (i.bollingerPosition < -0.85 && i.rsi7 < 36) side = "Long";
      if (i.bollingerPosition > 0.85 && i.rsi7 > 64) side = "Short";
      break;
    case "rsi-reversal":
      if (i.rsi7 < 30 && current.close > current.open) side = "Long";
      if (i.rsi7 > 70 && current.close < current.open) side = "Short";
      break;
    case "macd-momentum":
      if (i.macdHistogram > 0 && forecast.compositeScore > 0) side = "Long";
      if (i.macdHistogram < 0 && forecast.compositeScore < 0) side = "Short";
      break;
    case "volume-expansion":
      if (i.volumeZ >= Math.max(0.25, config.minVolumeZ) && i.return5m > 0) side = "Long";
      if (i.volumeZ >= Math.max(0.25, config.minVolumeZ) && i.return5m < 0) side = "Short";
      break;
    case "rapid-reversal":
    ...
    case "squeeze-release":
      if (i.bollingerBandwidth < 0.0065 && i.volumeZ > 0.35 && i.return5m > 0) side = "Long";
      if (i.bollingerBandwidth < 0.0065 && i.volumeZ > 0.35 && i.return5m < 0) side = "Short";
      break;
    case "vwap-reclaim":
      if (previous.close <= i.vwap30 && current.close > i.vwap30) side = "Long";
      if (previous.close >= i.vwap30 && current.close < i.vwap30) side = "Short";
      break;
    case "vwap-rejection":
      if (current.low <= i.vwap30 && current.close > i.vwap30 && current.close > current.open) side = "Long";
      if (current.high >= i.vwap30 && current.close < i.vwap30 && current.close < current.open) side = "Short";
      break;
    case "range-fade": {
      const range = Math.max(0.01, i.priorRangeHigh - i.priorRangeLow);
      const position = (current.close - i.priorRangeLow) / range;
      if (position < 0.15 && i.rsi14 < 48) side = "Long";
      if (position > 0.85 && i.rsi14 > 52) side = "Short";
      break;
    }
    case "hourly-open": {
      const hour = latestHourCandles(history);
      const open = hour.at(0)?.open;
      if (open !== undefined && current.close > open && i.return15m > 0) side = "Long";
      if (open !== undefined && current.close < open && i.return15m < 0) side = "Short";
      break;
    }
    case "hourly-opening-range": {
      ...
      if (parts.minute >= 15 && first15.length >= 10) {
        ...
        if (current.close > high) side = "Long";
        if (current.close < low) side = "Short";
      }
      break;
    }
  ...
  if (!side) return null;
  if (config.invertSignal) side = side === "Long" ? "Short" : "Long";
  if (!biasMatches(side, config.bias)) return null;
  if (forecast.confidence < config.minConfidence) return null;
  if (Math.abs(forecast.compositeScore) < config.minAbsScore) return null;
  if (i.volumeZ < config.minVolumeZ) return null;
  if (Math.abs(i.return5m) * 100 < config.minMovePct && !config.strategyId.startsWith("rapid")) return null;
  if (Math.abs(i.extensionAtr) > config.maxExtensionAtr) return null;
  if (config.requireEmaAlignment) {
    if (side === "Long" && !(i.ema9 > i.ema21)) return null;
    if (side === "Short" && !(i.ema9 < i.ema21)) return null;
  }
  if (config.requireVolumeConfirmation) {
    if (i.volumeZ < 0.25) return null;
    if (side === "Long" && i.return3m <= 0) return null;
    if (side === "Short" && i.return3m >= 0) return null;
  }
  return side;
};
...
const evaluateTrade = (
  ...
  config: StrategyConfig,
  setup: string,
): { trade: StrategyTrade; exitIndex: number } | null => {
  ...
  let exitReason: StrategyTrade["exitReason"] = "Time";
  let mfePct = 0;
  let maePct = 0;
...
export function runStrategyBacktest(
  ...
  const average = (items: StrategyTrade[]): number | null => items.length ? items.reduce((sum, trade) => sum + trade.netReturnPct, 0) / items.length : null;
...
export const probabilityAboveStrike = (
  forecast: Forecast,
  strike: number,
...
export function buildBinaryLadder(
  forecast: Forecast,
  currentTime: number,
  strikeGap: number,
  levelCount: number,
  hypotheticalContractPrice: number,
): BinaryLadderRow[] {
...
interface HourGroup {
  candles: Array<{ candle: Candle; index: number; parts: PacificParts }>;
}

export function runBinaryBacktest(
  candles: Candle[],
  settings: AnalyzerSettings,
  ...
  sortedGroups.forEach((group) => {
    ...
    if (!openItem || !entryItem || !settlementItem || settlementItem.parts.minute < 55 || entryItem.index < 130) return;
    opportunities += 1;
    ...
    if (config.side === "Above") { side = "Above"; modelProbability = probabilityAbove; }
    else if (config.side === "Below") { side = "Below"; modelProbability = probabilityBelow; }
    else if (aboveEdge >= belowEdge) { side = "Above"; modelProbability = probabilityAbove; }
    else { side = "Below"; modelProbability = probabilityBelow; }
    if (modelProbability - config.contractPrice < config.minimumEdge) return;
