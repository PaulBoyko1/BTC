...
import type { Candle, Forecast } from "./types.js";
...
export function renderPriceChart(
  ...
  const points: ChartSeriesPoint[] = visible.map((candle, index) => ({
    time: candle.time,
    close: candle.close,
  ...
  const x = (index: number): number => padding.left + (index / Math.max(1, totalSlots - 1)) * plotWidth;
  const y = (price: number): number =>
    padding.top + ((maxPrice - price) / Math.max(0.0001, maxPrice - minPrice)) * plotHeight;
